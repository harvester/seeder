...
func (itv *InventoryTemplateValidator) Delete(request *admission.Request, oldObj runtime.Object) error {
	...
	if len(inventoryTemplateObj.OwnerReferences) > 0 {
		nestedClusterObj := &seederv1alpha1.NestedCluster{}
		if err := itv.client.Get(itv.ctx, apimachinerytypes.NamespacedName{Name: inventoryTemplateObj.OwnerReferences[0].Name, Namespace: inventoryTemplateObj.Namespace}, nestedClusterObj); err != nil {
			if apierrors.IsNotFound(err) {
				// cluster object is deleted
				return nil
			}
			return werror.NewBadRequest(fmt.Sprintf("error while looking up owning cluster object: %v", err))
		}

		// parent nested cluster is being deleted, allow deletion of inventory template to cascade, otherwise block deletion of inventory template as it's owned by another object
		if !nestedClusterObj.DeletionTimestamp.IsZero() {
			return nil
		}

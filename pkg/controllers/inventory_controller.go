...
type inventoryReconciler func(context.Context, *bmaasv1alpha1.Inventory) (ctrl.Result, error)
...
func (r *InventoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, reconciler := range reconcileList {
		return reconciler(ctx, inventoryObj)
	}
...
func (r *InventoryReconciler) manageBaseboardObject(ctx context.Context, i *bmaasv1alpha1.Inventory) (ctrl.Result, error) {
	...
	return ctrl.Result{}, r.Client.Status().Update(ctx, i)
}

// checkAndMarkNodeReady will check the power status of the BaseboardManagement Object and Mark the node ready
func (r *InventoryReconciler) checkAndMarkNodeReady(ctx context.Context, i *bmaasv1alpha1.Inventory) (ctrl.Result, error) {
...
func (r *InventoryReconciler) handleBaseboardDeletion(ctx context.Context, i *bmaasv1alpha1.Inventory) (ctrl.Result, error) {
	...
	if i.Status.Status != "" {
		...
		if !b.DeletionTimestamp.IsZero() && controllerutil.ContainsFinalizer(b, bmaasv1alpha1.InventoryFinalizer) {
			...
			err = r.Delete(ctx, b)
			if err != nil {
				r.Error(err, "error deleting up baseboard object")
				return ctrl.Result{}, err
			}
		}
		// reset status to re-trigger recreation of baseboard objects
		i.Status.Status = ""
		return ctrl.Result{}, r.Status().Update(ctx, i)
	}

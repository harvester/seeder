...
var _ = Describe("reconcile out of band machine power operation", func() {
	var i *seederv1alpha1.Inventory
	var creds *corev1.Secret
	...
	It("perform inventory and machine reconciles", func() {
		...
		for _, expectedPowerState := range []rufio.PowerState{rufio.On, rufio.Unknown, rufio.Off} {
			By("update machine power state", func() {
				Eventually(func() error {
					b := &rufio.Machine{}
					err := k8sClient.Get(ctx, types.NamespacedName{Namespace: i.Namespace, Name: i.Name}, b)
					if err != nil {
			...
			By("reconcile inventory machine power state", func() {
				Eventually(func() error {
					iObj := &seederv1alpha1.Inventory{}
					err := k8sClient.Get(ctx, types.NamespacedName{Namespace: i.Namespace, Name: i.Name}, iObj)
					if err != nil {
					...
					if iObj.Status.MachinePowerState != expectedPowerState {
						return fmt.Errorf("waiting for inventory machine power state to be updated. Current state %v", iObj.Status.MachinePowerState)
					}

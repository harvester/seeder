...
RUN export K8S_VERSION=v1.34.0 && \
    ...
    read -r EXPECTED_HASH < <(yq -e '.releases[env(K8S_VERSION)][env(ENVTEST)].hash' /tmp/envtest.sha) && \
    ...
    read -r HASH < <(shasum -a 512 envtest-bins.tar.gz | awk '{print $1}') && \
